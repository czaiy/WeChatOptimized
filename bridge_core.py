...
参考 Akasha-WeChat bridge_core.py
...
import state
import config
from ob_protocol import make_message_event
...
class WeFlowBridge:
    ...
    def listen_sse(self):
        ...
        try:
            ...
            if self._sse_session.status_code != 200:
                log.error(f"[SSE] 连接失败: HTTP {self._sse_session.status_code}")
                return
    ...
    def process_image_message(self, data):
        ...
        try:
            ...
            caption_text = caption if caption else "（图片内容无法描述）"
            if caption:
...
def _push_event(event: dict) -> bool:
