...
WE_FLOW_BASE_URL = config.get("weflow_base_url", "http://127.0.0.1:5031")
ACCESS_TOKEN = config.get("access_token", "")
ASTRBOT_ATTACHMENTS = config.get("astrbot_attachments", "")
BOT_NICKNAMES = config.get("bot_nicknames", [])
BOT_WXID = config.get("bot_wxid", "")
SEND_METHOD = config.get("send_method", "uia")
WE_FLOW_SEND_API = config.get("weflow_send_api", "http://127.0.0.1:5031/api/v1/message")
BUFFER_SECONDS = config.get("buffer_seconds", 5)
WEB_PORT = config.get("web_port", 8766)
GROUP_REPLY_MODE = config.get("group_reply_mode", "mention")  # "mention" / "all" / "batch"
SENDER_SEARCH_ENABLED = config.get("sender_search_enabled", True)

# 鼠标漫游配置
WANDERER_ENABLED = config.get("wanderer_enabled", True)
...
ASTRBOT_OB_URL = config.get("astrbot_ob_url", "ws://127.0.0.1:11229/ws")
