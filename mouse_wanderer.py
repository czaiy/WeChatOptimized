"""
鼠标自然漫游模块
参考 WeeMessenger mouse_wanderer.py
...
logger = logging.getLogger("MouseWanderer")
...
class MouseWanderer:
    ...
    def _human_mouse_move(start_x: int, start_y: int, end_x: int, end_y: int):
        """模拟带有随机加速度和偏角的真人化鼠标移动轨迹（贝塞尔曲线模拟）"""
        try:
    ...
    def _random_wander(self, times: int = 1):
        ...
        for _ in range(times):
            ...
            # 若目标太近则重新选取
            if abs(cur_x - end_x) < 100 and abs(cur_y - end_y) < 100:
                end_x = random.randint(0, screen_width)
                end_y = random.randint(0, screen_height)
            self._human_mouse_move(cur_x, cur_y, end_x, end_y)
            time.sleep(random.uniform(0.2, 1.5))

    def _run_loop(self):
        """鼠标漫游主循环（在独立线程中运行）"""
        logger.info("[鼠标漫游] 线程已启动")
        try:
            import uiautomation as auto
            with auto.UIAutomationInitializerInThread():
                while not self._stop_event.is_set():
                    ...
                    try:
                        ...
                        logger.warning(f"[鼠标漫游] 异常: {e}")
        except ImportError:
            logger.error("[鼠标漫游] uiautomation 未安装，鼠标漫游不可用")
